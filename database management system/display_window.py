...
class display_window(QWidget):
   def __init__(self, ar_list, ar_image):
      ...
      print(self.attr_list[0], self.attr_list[1])
   ...
   def click_to_edit(self):
      self.admin = Authenticate_admin()
      self.admin.show()
      print(self.admin.button_status)
      if self.admin.button_status == True:
         value = self.admin.log_details()
         print(value)
         if value == True:
            if 'officer' in ar_image:
               self.officer_display = edit_officer()
               self.officer_display.show()
               self.admin.destroy()
            elif 'soldier' in ar_image:
               self.soldier_display = edit_soldier()
               self.soldier_display.show()
               self.admin.destroy()
...
class edit_officer():
   def __init__(self,my_list, my_image):
      self.attr = my_list
      self.image = my_image
      self.background()
...
#       self.rank_title_edit.setPlaceholderText("Enter Army number")
   ...
   def save_officer(self):
        ...
        self.data_d.update_records(base_type = 'officer')
...
class edit_soldier():
   ...
   def save_soldier(self):
        ...
        self.data_d = Database()
        self.data_d.data = unique_list
        self.data_d.update_records(base_type = 'soldier')
        print('written to database')
        return
