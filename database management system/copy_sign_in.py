...
class selector(QWidget):
   def __init__(self):
      super().__init__()
      self.title = 'select officer/soldier'
      self.top = 400
      self.left = 300
      self.width = 200
      self.height = 100
   ...
   def display_buttons(self):
      layout = QGridLayout()
      self.soldier = QPushButton('Soldier', self)
      layout.addWidget(self.soldier,3,2,3,3)
      self.soldier.resize(150,40)
      #self.soldier.move(240,350)
      self.soldier.setToolTip("Create a new soldier's profile")
      self.soldier.setStyleSheet('background-color:#4e4e4e;color:#f7f7f7;')
      self.soldier.clicked.connect(self.show_soldier)
      
      
      self.officer = QPushButton('Officer', self)
      layout.addWidget(self.officer,3,7,3,3)
      self.officer.resize(150,40)
      #self.officer.move(240,350)
      self.officer.setToolTip("Create a new officer's profile")
      self.officer.setStyleSheet('background-color:#4e4e4e;color:#f7f7f7;')
      self.officer.clicked.connect(self.show_officer)
      
      self.close_button = QPushButton('close', self)
      layout.addWidget(self.close_button,3,12,3,3)
      self.close_button.resize(150,40)
      #self.officerself.close_button.move(240,350)
      self.close_button.setToolTip("Create a new officer's profile")
      self.close_button.setStyleSheet('background-color:#4e4e4e;color:#f7f7f7;')
      self.close_button.clicked.connect(self.close_me)
...
class sign_in(QWidget):
    ...
    def format_this(self, value):
       ...
       if '/' in value:
          ...
          while '/' in value_to_list:
             value_to_list.remove('/')
          for k in value_to_list:
             name += k
       else:
          name = value
       return name
    ...
    def add_image(self):
       ...
       new_path = r'C:\Users\ACER\Desktop\database_manager\images\\'
       self.get_image.save(new_path + self.rank_title_edit.text()+ '_' + self.format_this(str(self.army_type)) + '.png')
       print ('image saved')
